...
#endif
  typedef FABDEF    *p32fab_t;
  typedef RAB64DEF  *p32rab64_t;
  typedef RABDEF    *p32rab_t;
  typedef NAMLDEF   *p32naml_t;
  typedef XABSUMDEF *p32xabsum_t;
  typedef XABALLDEF *p32xaball_t;
  typedef XABKEYDEF *p32xabkey_t;
  typedef XABFHCDEF *p32xabfhc_t;
...
typedef struct {
  ...
  p32xaball_t   parea;
  p32xabkey_t   pkey;
  __char_ptr32     plong_filename;
  __char_ptr32     pesa;
  __char_ptr32     plong_esa;
  __char_ptr32     prsa;
  __char_ptr32     plong_rsa;
  unsigned int flags;
} rms_file_t;
...
static void getmsg(unsigned int cond, char *buf, int buflen) {
  ...
  $DESCRIPTOR64(bufD, "");
  bufD.dsc64$q_length = buflen - 1;
  bufD.dsc64$pq_pointer = buf;
...
static void fill(rms_file_t *self, char *fn, int access, int share,
                 unsigned int fop) {
  self->pfab = (p32fab_t)malloc32(sizeof(FABDEF));
  *self->pfab = cc$rms_fab;
  self->prab = (p32rab64_t)malloc32(sizeof(RAB64DEF));
  *self->prab = cc$rms_rab64;
  self->psum = (p32xabsum_t)malloc32(sizeof(XABSUMDEF));
  *self->psum = cc$rms_xabsum;
  self->pnaml = (p32naml_t)malloc32(sizeof(NAMLDEF));
  *self->pnaml = cc$rms_naml;
  ...
  self->pfab->fab$l_fna = (p32void_t)-1;
  self->pfab->fab$b_fns = 0;
  self->pfab->fab$l_fop = fop;
  self->pfab->fab$l_xab = (p32void_t)self->psum;

  self->pfab->fab$l_naml = (p32void_t)self->pnaml;

  self->plong_filename = (__char_ptr32)malloc32(NAML$C_MAXRSS + 1);
  strncpy(self->plong_filename, fn, NAML$C_MAXRSS);
  self->plong_filename[NAML$C_MAXRSS] = '\0';

  self->pnaml->naml$l_long_filename = (p32void_t)self->plong_filename;
  self->pnaml->naml$l_long_filename_size = strlen(self->plong_filename);

  self->pesa = (__char_ptr32)malloc32(NAM$C_MAXRSS + 1);
  self->pnaml->naml$l_esa = (p32void_t)self->pesa;
  self->pnaml->naml$b_ess = NAM$C_MAXRSS;
  self->prsa = (__char_ptr32)malloc32(NAM$C_MAXRSS + 1);
  self->pnaml->naml$l_rsa = (p32void_t)self->prsa;
  self->pnaml->naml$b_rss = NAM$C_MAXRSS;

  self->plong_esa = (__char_ptr32)malloc32(NAM$C_MAXRSS + 1);
  self->pnaml->naml$l_long_expand = (p32void_t)self->plong_esa;
  self->pnaml->naml$l_long_expand_alloc = NAM$C_MAXRSS;
  self->plong_rsa = (__char_ptr32)malloc32(NAM$C_MAXRSS + 1);
  self->pnaml->naml$l_long_result = (p32void_t)self->plong_rsa;
  self->pnaml->naml$l_long_result_alloc = NAM$C_MAXRSS;

  self->pnaml->naml$v_synchk = 0; // Have $PARSE do directory existence check
  self->pnaml->naml$v_no_short_upcase =
      1; // Don't uppercase short expanded file specification
}
...
static unsigned int _open(rms_file_t *self) {
  ...
  pxab_chain = self->pfab->fab$l_xab;

  i = self->psum->xab$b_nok;

  adc_Assert((self->pkey = pkey = (p32xabkey_t)malloc32(i * sizeof(XABKEYDEF))));
  while (--i >= 0) {
    *pkey = cc$rms_xabkey;
    pkey->xab$b_ref = i;
    pkey->xab$l_nxt = pxab_chain;
    pxab_chain = pkey;
    pkey++;
  }

  i = self->psum->xab$b_noa;
  adc_Assert((self->parea = parea = (p32xaball_t)malloc32(i * sizeof(XABALLDEF))));
  while (--i >= 0) {
    *parea = cc$rms_xaball;
    parea->xab$b_aid = i;
    parea->xab$l_nxt = pxab_chain;
    pxab_chain = parea;
    parea++;
  }
  ...
  self->prab->rab64$l_ubf = (p32void_t)-1;
  self->prab->rab64$pq_ubf = NULL;
  self->prab->rab64$w_usz = 0;
  self->prab->rab64$q_usz = self->pfab->fab$w_mrs;

  return (sys$connect((p32rab_t)self->prab));
}

static unsigned int _close(rms_file_t *self) {
  unsigned int status;

  status = sys$disconnect((p32rab_t)self->prab);
...
static unsigned int _flush(rms_file_t *self) {
  return (sys$flush((p32rab_t)self->prab));
}

static unsigned int _free(rms_file_t *self) {
  return (sys$free((p32rab_t)self->prab));
}

static unsigned int _release(rms_file_t *self) {
  return (sys$release((p32rab_t)self->prab));
}
...
static unsigned int _update(rms_file_t *self, char *buf, int len) {
  ...
  self->prab->rab64$pq_rbf = buf;
  self->prab->rab64$q_rsz = len;
  return (sys$update((p32rab_t)self->prab));
}

static unsigned int _fetch(rms_file_t *self, char *key, int len, char *buf,
                           int *retlen) {
  unsigned int status;

  if (key) {
    self->prab->rab64$l_kbf = (p32void_t)-1;
  ...
  self->prab->rab64$l_ubf = (p32void_t)-1;
  self->prab->rab$pq_ubf = buf;

  status = sys$get((p32rab_t)self->prab);

  if (!OKAY(status)) {
    return (status);
  }

  *retlen = self->prab->rab$q_rsz;
  return (status);
}
...
static unsigned int _find(rms_file_t *self, char *key, int len) {
  ...
  return (sys$find((p32rab_t)self->prab));
}
...
static unsigned int _delete(rms_file_t *self, char *key, int len) {
  ...
  if (key) {
    ...
    status = sys$find((p32rab_t)self->prab);
  ...
  return (sys$delete((p32rab_t)self->prab));
}
...
static PyObject *iternext(PyFileObject *self) {
  ...
  if (!((rms_file_t *)self)->pfab) {
    return PyErr_NoMemory();
  }

  char *pbuf = alloca(((rms_file_t *)self)->pfab->fab$w_mrs);

  status = _fetch((rms_file_t *)self, NULL, 0, pbuf, &len);
  ...
  if (status == RMS$_EOF) {
    ...
    obj = PyBytes_FromStringAndSize(pbuf, len);
  }

  return (obj);
}

static void free_fo_fields(rms_file_t *self) {
  free(self->parea);
  free(self->pesa);
  free(self->pfab);
  free(self->pkey);
...
static PyObject *RMS_rewind(rms_file_t *self, PyObject *args,
  ...
  char msg[256];
  PyObject *obj;
  char *kwnames[] = {"keynum", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwargs, (char *)"|i:rewind", kwnames,
                                   &keynum)) {
    return (NULL);
  }
  ...
  status = sys$rewind((p32rab_t)self->prab, 0, 0);
...
static PyObject *RMS_usekey(rms_file_t *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, (char *)"|i:usekey", kwnames,
                                   &keynum)) {
    return (NULL);
  }
...
static PyObject *RMS_put(rms_file_t *self, PyObject *args, PyObject *kwargs) {
  ...
  PyObject *res, *obj;
  char msg[256];
  char *kwnames[] = {"record", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwargs, (char *)"s#:put", kwnames,
                                   &buf, &len)) {
    return (NULL);
  }
...
static PyObject *RMS_update(rms_file_t *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, (char *)"s#:update", kwnames,
                                   &buf, &len)) {
    return (NULL);
  }
...
static PyObject *RMS_fetch(rms_file_t *self, PyObject *args, PyObject *kwargs) {
  ...
  char msg[256];
  char *kwnames[] = {"key", NULL};

  if (!self->pfab) {
    return PyErr_NoMemory();
  }

  char *pbuf = alloca(self->pfab->fab$w_mrs);
  int retlen = 0;

  if (!PyArg_ParseTupleAndKeywords(args, kwargs, (char *)"|s#:fetch", kwnames,
                                   &keyval, &len)) {
    return (NULL);
  }

  status = _fetch((rms_file_t *)self, keyval, len, pbuf, &retlen);
  ...
  if (status == RMS$_EOF || status == RMS$_RNF) {
    ...
    o2 = PyBytes_FromStringAndSize(pbuf, retlen);
  }
...
static PyObject *RMS_find(rms_file_t *self, PyObject *args, PyObject *kwargs) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, (char *)"|s#:find", kwnames,
                                   &keyval, &len)) {
    return (NULL);
  }
...
static PyObject *RMS_delete(rms_file_t *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, (char *)"|s#:delete", kwnames,
                                   &keyval, &len)) {
    return (NULL);
  }
...
static PyObject *RMS_setrop(rms_file_t *self, PyObject *args,
  ...
  unsigned int status;
  int rop = 0;
  char *kwnames[] = {"rop", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwargs, (char *)"i:setrop", kwnames,
                                   &rop)) {
    return (NULL);
  }
...
static PyGetSetDef rms_getsetters[] = {
    {"longname", (getter) RMS_getlongname, NULL, "long filename", NULL},
    {"nok", (getter) RMS_getnok, NULL, "number of keys", NULL},
    {"org", (getter) RMS_getorg, NULL, "file organization", NULL},
    {NULL}  /* Sentinel */
};
...
static PyObject *RMS_new(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  char msg[256];
  PyObject *obj;
  char *kwnames[] = {"name", "fac", "shr", "fop", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwargs, (char *)"s|iiI:open_file",
                                   kwnames, &name, &fac, &shr, &fop)) {
    return (NULL);
  }
...
static PyObject *tuple2bcd(PyObject *dummy, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "(iO!i)ii", &sign, &PyTuple_Type, &lst, &dprec,
                        &n, &prec)) {
    return (NULL);
  }
...
static unsigned int _getrmsattr(char *name, int attr, int *res) {
  ...
  if (attr == RMSATTR_K_LRL) {
    adc_Assert((fhc = (p32xabfhc_t)malloc32(sizeof(XABFHCDEF))));
    *fhc = cc$rms_xabfhc;
...
static unsigned int _parse(char *path, char *node, char *dev, char *dir,
                           char *name, char *type, char *ver) {
  unsigned int status;
  ...
  *node = *dev = *dir = *name = *type = *ver = '\0';
  ...
  if (p6) {
    strncpy(ver, fo->pnaml->naml$l_long_expand + p1 + p2 + p3 + p4 + p5, p6);
    ver[p6] = '\0';
  }
...
PyTypeObject RmsFile_Type = {
    PyObject_HEAD_INIT(NULL) "rmsFile", // tp_name
...
PyMODINIT_FUNC PyInit__rms(void) {
  ...
  if (RMS_error == NULL) {
    if ((RMS_error = PyErr_NewException("vms.rms.error", PyExc_IOError,
                                        dict)) == NULL) {
      return (NULL);
    }
  }
