            ...
            if file == '_sysconfigdata__OpenVMS_cpython-310-alpha-openvms.py':
                ...
                for idx, line in enumerate(lines):
                    if 'SIZEOF_VOID_P' in line:
                        if line != " 'SIZEOF_VOID_P': 8,\n":
                            lines[idx] = " 'SIZEOF_VOID_P': 8,\n"
                            rewrite = True
                        break
                if rewrite:
...
--
product VSI AXPVMS PYTHON64 {type}{major}.{minor}-{level}{edit} FULL ;
...
'''
    ...
    kit_content = kit_template.format(
        ...
        dirs='\n    '.join(all_dirs),
        files='\n    '.join(all_files))
    with open('python64.pcsi$desc', 'w') as file:
        file.write(kit_content)

    text_template = '''=product VSI AXPVMS PYTHON64 {type}{major}.{minor}-{level}{edit} full
1 'PRODUCT
...
'''
    text_content = text_template.format(
        ...
        dirs='\n    '.join(all_dirs),
        files='\n    '.join(all_files))
    with open('python64.pcsi$text', 'w') as file:
